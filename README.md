...
- **Claude 側** … ローカルの Claude Code CLI(`claude -p`)を、stream-json 入出力で**議論全体を通して1プロセス**として動かします。CLI の起動には 9 秒ほどかかるため、発言のたびに起動し直すと Claude 側の大半が起動待ちになります(実測: 1発言 9.3 秒 → 1.7 秒)。プロセスが途中で死んだ場合は、記録済みの session id を `--resume` に渡して1度だけ自動復帰します。Claude Code は本来コーディング用アシスタントなので、そのままだと非技術系のテーマを「職務範囲外」と断ることがあります(実測: haiku で発生)。そのため毎回 `--append-system-prompt` で「この討論への参加があなたのタスク」と明示しています。
